...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($passwordError) {
        $message = "<div class='error'>{$passwordError}</div>";
    }

    // Validate profile picture
    $profilePictureError = $validation->validateProfilePicture($_FILES['profile_picture']);
    if ($profilePictureError) {
        $message = "<div class='error'>{$profilePictureError}</div>";
    }
